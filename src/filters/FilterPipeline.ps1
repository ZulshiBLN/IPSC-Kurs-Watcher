...
function Invoke-FilterPipeline { param([object[]]$Courses, [object]$FilterConfig)
    ...
    $result = $Courses
    if ($FilterConfig.course_types) { $result = Filter-ByType -Courses $result -TypeFilters $FilterConfig.course_types }
    if ($FilterConfig.exclude_patterns) { $result = Filter-ByExclusion -Courses $result -ExcludePatterns $FilterConfig.exclude_patterns }
